...
save_img_or_flash(){
	local choice
	choice=$(zenity --list --title="Save or Flash" --text="Do you want to save the image or write it to a USB flash drive?" --column="Choice" "Save Image" "Write to USB Flash Drive") || exit 1

	if [[ "$choice" = "Save Image" ]]; then
		local cros_bin_name
		cros_bin_name="$(jq -r '.file' <<<"$cros_selected_json")"
		local save_path
		save_path=$(zenity --file-selection --save --filename="$cros_bin_name" --title="Save Chrome OS Image As") || exit 1
		cp "$work_dir/$cros_bin_name" "$save_path"
		zenity --info --text="イメージが保存されました: $save_path"
	else
		select_usb_flash
		run_dd
	fi
